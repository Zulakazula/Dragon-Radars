...
public class DormantRadarScreen extends Screen {
    ...
    private List<TrackedDragon> getFilteredList() {
        ...
        return result.stream()
            ...
            .sorted((d1, d2) -> {
                ...
                boolean d2Tracked = d2.id.equals(currentlyTrackedDormant);
    ...
    protected void init() {
        this.addRenderableWidget(new TransparentButton(10, 40, 110, 20, Component.literal(""), b -> {
            if (!DormantRadarSettings.INSTANCE.hasPerformedSearch && !isWaitingForResults) performActualSearch();
                ...
                if (selectedDragonEntry.equals(currentlyTrackedDormant)) {
                    currentlyTrackedDormant = null;
                    ...
                    currentlyTrackedDormant = selectedDragonEntry;
                    GlobalRadarState.startTracking("dormant", selectedDragonEntry);
                    this.onClose();
                }
            } else if (currentlyTrackedDormant != null) {
                currentlyTrackedDormant = null;
                GlobalRadarState.stopTracking();
            } else { 
                DormantRadarSettings.INSTANCE.globalResults.clear(); 
            ...
            @Override public void renderWidget(GuiGraphics gui, int mx, int my, float pt) {
                ...
                if (isWaitingForResults) {
                    ...
                    label = "Start Search";
                } else if (currentlyTrackedDormant != null && (selectedDragonEntry == null || selectedDragonEntry.equals(currentlyTrackedDormant))) {
                    label = "Stop Tracking";
                } else if (selectedDragonEntry != null) {
        ...
        this.addRenderableWidget(new TransparentButton(10, 65, 110, 20, Component.literal("Refresh List"), b -> {
            performActualSearch(); this.refreshFeedbackTicks = 40; currentlyTrackedDormant = null; this.selectedDragonEntry = null;
        }) {
            @Override public void renderWidget(GuiGraphics gui, int mx, int my, float pt) {
        ...
        this.addRenderableWidget(new TransparentButton(this.width - 120, 10, 110, 20, Component.literal("Teleport"), b -> {
            if (this.minecraft.player != null && currentlyTrackedDormant != null) {
                getFilteredList().stream().filter(d -> d.id.equals(currentlyTrackedDormant)).findFirst().ifPresent(td -> {
            ...
            @Override public void renderWidget(GuiGraphics gui, int mx, int my, float pt) {
                this.active = minecraft.player != null && minecraft.player.isCreative() && currentlyTrackedDormant != null;
                super.renderWidget(gui, mx, my, pt);
            }
        });
        ...
        if (isWaitingForResults) {
            ...
            if (filtered.isEmpty() && DormantRadarSettings.INSTANCE.hasPerformedSearch) {
                ...
                for (int i = 0; i < filtered.size(); i++) {
                    ...
                    renderDragonProfile(guiGraphics, d, rX, rY, d.id.equals(selectedDragonEntry), d.id.equals(currentlyTrackedDormant), hover);
                }
   ...
   private void renderStatsBox(GuiGraphics gui, int x, int y) {
    ...
    public boolean mouseClicked(double mx, double my, int b) { 
        ...
        if (!DormantRadarSettings.INSTANCE.globalResults.isEmpty() && mx >= sBX - 5 && mx <= sBX + 15) {
            int barPos = (int)((getMaxScroll() <= 0) ? 0 : (scrollAmount / getMaxScroll()) * (lH - 40));
            if (my >= lSY + barPos && my <= lSY + barPos + 40) {
    ...
    public void tick() { 
        super.tick(); 
        if (isWaitingForResults) {
            if (!DormantRadarSettings.INSTANCE.globalResults.isEmpty() || DragonScanner.isSearchComplete) { 
    ...
    public boolean mouseScrolled(double mx, double my, double d) { 
        this.scrollAmount = Mth.clamp(this.scrollAmount - d * 25, 0, getMaxScroll()); 
        return true; 
    }
}