...
public class IceRadarScreen extends Screen {
    ...
    protected void init() {
        this.addRenderableWidget(new TransparentButton(10, 40, 110, 20, Component.literal(""), b -> {
            if (!IceRadarSettings.INSTANCE.hasPerformedSearch && !isWaitingForResults) performActualSearch();
                ...
                if (selectedDragonEntry.equals(currentlyTrackedIce)) {
                    currentlyTrackedIce = null;
                    ...
                    currentlyTrackedIce = selectedDragonEntry;
                    GlobalRadarState.startTracking("ice", selectedDragonEntry);
                    this.onClose();
                }
            } else if (currentlyTrackedIce != null) {
                currentlyTrackedIce = null;
                GlobalRadarState.stopTracking();
            } else { 
                IceRadarSettings.INSTANCE.globalResults.clear(); 
            ...
            @Override public void renderWidget(GuiGraphics gui, int mx, int my, float pt) {
                ...
                if (isWaitingForResults) {
                    ...
                    label = "Start Search";
                } else if (currentlyTrackedIce != null && (selectedDragonEntry == null || selectedDragonEntry.equals(currentlyTrackedIce))) {
                    label = "Stop Tracking";
                } else if (selectedDragonEntry != null) {
        ...
        this.addRenderableWidget(new TransparentButton(10, 65, 110, 20, Component.literal("Refresh List"), b -> {
            performActualSearch(); this.refreshFeedbackTicks = 40; currentlyTrackedIce = null; this.selectedDragonEntry = null;
        }) {
            @Override public void renderWidget(GuiGraphics gui, int mx, int my, float pt) {
        ...
        this.addRenderableWidget(new TransparentButton(10, 200, 110, 20, Component.literal(""), b -> IceRadarSettings.INSTANCE.sortClosest = !IceRadarSettings.INSTANCE.sortClosest) {
            @Override public void renderWidget(GuiGraphics gui, int mx, int my, float pt) {
        ...
        this.addRenderableWidget(new TransparentButton(this.width - 120, 10, 110, 20, Component.literal("Teleport"), b -> {
            if (this.minecraft.player != null && currentlyTrackedIce != null) {
                getFilteredList().stream().filter(d -> d.id.equals(currentlyTrackedIce)).findFirst().ifPresent(td -> {
            ...
            @Override public void renderWidget(GuiGraphics gui, int mx, int my, float pt) {
                this.active = minecraft.player != null && minecraft.player.isCreative() && currentlyTrackedIce != null;
                super.renderWidget(gui, mx, my, pt);
            }
        });
        ...
        if (isWaitingForResults) {
            ...
            if (filtered.isEmpty() && IceRadarSettings.INSTANCE.hasPerformedSearch) {
                ...
                for (int i = 0; i < filtered.size(); i++) {
                    ...
                    renderDragonProfile(guiGraphics, d, rX, rY, d.id.equals(selectedDragonEntry), d.id.equals(currentlyTrackedIce), hover);
                }
    ...
    private void renderStatsBox(GuiGraphics gui, int x, int y) {
        ...
        gui.drawString(this.font, "Total = " + ice, sx, sy + 27, 0xFFFFFF);
        ps.popPose();
    }
    ...
    @Override public boolean mouseClicked(double mx, double my, int b) { 
        ...
        if (!IceRadarSettings.INSTANCE.globalResults.isEmpty() && mx >= sBX - 5 && mx <= sBX + 15 && my >= lSY && my <= lSY + lH) { this.isDraggingScrollBar = true; return true; } 
    ...
    @Override public void tick() { 
        super.tick(); 
        if (isWaitingForResults) {
            if (!IceRadarSettings.INSTANCE.globalResults.isEmpty() || DragonScanner.isSearchComplete) { isWaitingForResults = false; IceRadarSettings.INSTANCE.hasPerformedSearch = true; } 
            else { serverWaitTimer--; if (serverWaitTimer <= 0) { isWaitingForResults = false; IceRadarSettings.INSTANCE.hasPerformedSearch = true; } }
        }
