...
@OnlyIn(Dist.CLIENT)
public class IceRadarScreen extends Screen {
    ...
    public IceRadarScreen() {
        ...
        if (IceRadarSettings.INSTANCE.searchRadius <= 0 || IceRadarSettings.INSTANCE.searchRadius > 15000) 
            IceRadarSettings.INSTANCE.searchRadius = 7500;
        if (IceRadarSettings.INSTANCE.selectedStages.isEmpty()) 
            IceRadarSettings.INSTANCE.selectedStages.addAll(Arrays.asList(stageNames));
        if (IceRadarSettings.INSTANCE.selectedGenders.isEmpty()) 
            IceRadarSettings.INSTANCE.selectedGenders.addAll(Arrays.asList(genderNames));
    }

    private void performActualSearch() {
        if (this.minecraft.player == null) return;
        IceRadarSettings.INSTANCE.globalResults.clear();
        IceRadarSettings.INSTANCE.hasPerformedSearch = false; 
    ...
    private List<TrackedDragon> getFilteredList() {
        int currentHash = Objects.hash(
            IceRadarSettings.INSTANCE.selectedStages,
            IceRadarSettings.INSTANCE.selectedGenders,
            IceRadarSettings.INSTANCE.sortClosest,
            IceRadarSettings.INSTANCE.globalResults.size()
        );
    ...
    private List<TrackedDragon> calculateFilteredList() {
        ...
        for (int i = 0; i < IceRadarSettings.INSTANCE.globalResults.size(); i++) {
            DragonInfo info = IceRadarSettings.INSTANCE.globalResults.get(i);
        ...
        return result.stream()
            ...
            .sorted((d1, d2) -> {
                boolean d1Tracked = d1.id.equals(ClientRadarState.currentlyTrackedIce);
                boolean d2Tracked = d2.id.equals(ClientRadarState.currentlyTrackedIce);
    ...
    protected void init() {
        this.addRenderableWidget(new TransparentButton(10, 40, 110, 20, Component.literal(""), b -> {
            if (!IceRadarSettings.INSTANCE.hasPerformedSearch && !isWaitingForResults) {
                // Block if another radar is searching
                if (DragonScanner.currentlySearchingRadar != null && !DragonScanner.currentlySearchingRadar.equals("ice")) {
            ...
            else if (isWaitingForResults) { this.isWaitingForResults = false; IceRadarSettings.INSTANCE.hasPerformedSearch = true; }
            else if (selectedDragonEntry != null) {
                if (selectedDragonEntry.equals(ClientRadarState.currentlyTrackedIce)) {
                    ...
                    ClientRadarState.currentlyTrackedIce = selectedDragonEntry;
                    GlobalRadarState.startTracking("ice", selectedDragonEntry);
                    this.onClose();
                }
            } else if (ClientRadarState.currentlyTrackedIce != null) {
                ClientRadarState.currentlyTrackedIce = null;
                GlobalRadarState.stopTracking();
            } else {
                IceRadarSettings.INSTANCE.globalResults.clear();
            ...
            @Override public void renderWidget(GuiGraphics gui, int mx, int my, float pt) {
                ...
                } else if (!IceRadarSettings.INSTANCE.hasPerformedSearch) {
                    label = "Start Search";
                } else if (ClientRadarState.currentlyTrackedIce != null && (selectedDragonEntry == null || selectedDragonEntry.equals(ClientRadarState.currentlyTrackedIce))) {
                    label = "Stop Tracking";
                } else if (selectedDragonEntry != null) {
                    if (GlobalRadarState.isTracking() && !GlobalRadarState.isTrackingWith("ice")) {
                        label = "Switch Target";
                    } else {
        ...
        this.addRenderableWidget(new TransparentButton(10, 65, 110, 20, Component.literal("Refresh List"), b -> {
            performActualSearch(); this.refreshFeedbackTicks = 40; ClientRadarState.currentlyTrackedIce = null; this.selectedDragonEntry = null;
        }) {
            @Override public void renderWidget(GuiGraphics gui, int mx, int my, float pt) {
        ...
        this.addRenderableWidget(new TransparentButton(10, 155, 110, 20, Component.literal("Reset Filters"), b -> { 
            IceRadarSettings.INSTANCE.selectedStages.clear(); IceRadarSettings.INSTANCE.selectedStages.addAll(Arrays.asList(stageNames));
            IceRadarSettings.INSTANCE.selectedGenders.clear(); IceRadarSettings.INSTANCE.selectedGenders.addAll(Arrays.asList(genderNames));
            IceRadarSettings.INSTANCE.searchRadius = 7500; IceRadarSettings.INSTANCE.sortClosest = true;
        ...
        this.addRenderableWidget(new TransparentButton(10, 200, 110, 20, Component.literal(""), b -> {
            IceRadarSettings.INSTANCE.sortClosest = !IceRadarSettings.INSTANCE.sortClosest;
            this.cachedFilteredList = null;
        }) {
            @Override public void renderWidget(GuiGraphics gui, int mx, int my, float pt) {
                this.setMessage(Component.literal(IceRadarSettings.INSTANCE.sortClosest ? "Dist: Closest -> Far" : "Dist: Far -> Closest"));
                super.renderWidget(gui, mx, my, pt);
            }
        });

        this.radiusSlider = new TransparentSlider(10, 242, 110, 20, Component.literal("Radius: " + IceRadarSettings.INSTANCE.searchRadius), (double) IceRadarSettings.INSTANCE.searchRadius / 15000.0D) { 
            @Override protected void updateMessage() { IceRadarSettings.INSTANCE.searchRadius = Math.max(100, Math.round(((int)(this.value * 15000.0D)) / 100.0f) * 100); this.setMessage(Component.literal("Radius: " + IceRadarSettings.INSTANCE.searchRadius)); } 
        ...
        this.addRenderableWidget(new TransparentButton(this.width - 120, 10, 110, 20, Component.literal("Teleport"), b -> {
            if (this.minecraft.player != null && ClientRadarState.currentlyTrackedIce != null) {
                getFilteredList().stream().filter(d -> d.id.equals(ClientRadarState.currentlyTrackedIce)).findFirst().ifPresent(td -> {
            ...
            @Override public void renderWidget(GuiGraphics gui, int mx, int my, float pt) {
                this.active = minecraft.player != null && minecraft.player.isCreative() && ClientRadarState.currentlyTrackedIce != null;
                super.renderWidget(gui, mx, my, pt);
            }
        });
    ...
    public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTicks) {
        ...
        if (isWaitingForResults) {
            ...
            if (IceRadarSettings.INSTANCE.hasPerformedSearch) renderStatsBox(guiGraphics, statsX, 40);

            List<TrackedDragon> filtered = getFilteredList();
            if (filtered.isEmpty() && IceRadarSettings.INSTANCE.hasPerformedSearch) {
                ...
                for (int i = firstVisibleIndex; i < lastVisibleIndex; i++) {
                    ...
                    renderDragonProfile(guiGraphics, d, rX, rY, d.id.equals(selectedDragonEntry), d.id.equals(ClientRadarState.currentlyTrackedIce), hover);
                }
        ...
        if (showStageBox) renderPopupBox(guiGraphics, stageNames, IceRadarSettings.INSTANCE.selectedStages, 125, 105);
        if (showGenderBox) renderPopupBox(guiGraphics, genderNames, IceRadarSettings.INSTANCE.selectedGenders, 125, 130);
    }
    ...
    private void renderStatsBox(GuiGraphics gui, int x, int y) {
        long ice = IceRadarSettings.INSTANCE.globalResults.stream().filter(d -> d.type.contains("Ice")).count();
    ...
    public boolean mouseClicked(double mx, double my, int b) { 
        ...
        int maxScroll = getMaxScroll();
        if (maxScroll > 0 && !IceRadarSettings.INSTANCE.globalResults.isEmpty() && mx >= sBX - 5 && mx <= sBX + 15) {
        ...
        if (showStageBox && handlePopupClick(mx, my, stageNames, IceRadarSettings.INSTANCE.selectedStages, 125, 105)) return true; 
        if (showGenderBox && handlePopupClick(mx, my, genderNames, IceRadarSettings.INSTANCE.selectedGenders, 125, 130)) return true;
    ...
    public void tick() { 
        super.tick(); 
        if (isWaitingForResults) {
            if (!IceRadarSettings.INSTANCE.globalResults.isEmpty()) {
