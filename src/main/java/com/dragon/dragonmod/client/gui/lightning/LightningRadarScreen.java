...
public class LightningRadarScreen extends Screen {
    ...
    private List<TrackedDragon> getFilteredList() {
        int currentHash = Objects.hash(
            LightningRadarSettings.INSTANCE.selectedStages,
            LightningRadarSettings.INSTANCE.selectedGenders,
            LightningRadarSettings.INSTANCE.sortClosest,
            LightningRadarSettings.INSTANCE.globalResults.size()
        );
    ...
    private List<TrackedDragon> calculateFilteredList() {
        ...
        return result.stream()
            .filter(d -> LightningRadarSettings.INSTANCE.selectedStages.contains("Stage " + d.stage))
            .filter(d -> LightningRadarSettings.INSTANCE.selectedGenders.contains(d.isMale ? "Male" : "Female"))
            .sorted((d1, d2) -> {
                boolean d1Tracked = d1.id.equals(ClientRadarState.currentlyTrackedLightning);
                boolean d2Tracked = d2.id.equals(ClientRadarState.currentlyTrackedLightning);
    ...
    protected void init() {
        this.addRenderableWidget(new TransparentButton(10, 40, 110, 20, Component.literal(""), b -> {
            if (!LightningRadarSettings.INSTANCE.hasPerformedSearch && !isWaitingForResults) {
                // Block if another radar is searching
                if (DragonScanner.currentlySearchingRadar != null && !DragonScanner.currentlySearchingRadar.equals("lightning")) {
                ...
                if (selectedDragonEntry.equals(ClientRadarState.currentlyTrackedLightning)) {
                    ...
                    ClientRadarState.currentlyTrackedLightning = selectedDragonEntry;
                    GlobalRadarState.startTracking("lightning", selectedDragonEntry);
                    this.onClose();
                }
            } else if (ClientRadarState.currentlyTrackedLightning != null) {
                ClientRadarState.currentlyTrackedLightning = null;
                GlobalRadarState.stopTracking();
            } else {
                LightningRadarSettings.INSTANCE.globalResults.clear();
            ...
            @Override public void renderWidget(GuiGraphics gui, int mx, int my, float pt) {
                ...
                if (isWaitingForResults) {
                    ...
                    label = "Start Search";
                } else if (ClientRadarState.currentlyTrackedLightning != null && (selectedDragonEntry == null || selectedDragonEntry.equals(ClientRadarState.currentlyTrackedLightning))) {
                    label = "Stop Tracking";
                } else if (selectedDragonEntry != null) {
        ...
        this.addRenderableWidget(new TransparentButton(10, 65, 110, 20, Component.literal("Refresh List"), b -> {
            performActualSearch(); this.refreshFeedbackTicks = 40; ClientRadarState.currentlyTrackedLightning = null; this.selectedDragonEntry = null;
        }) {
            @Override public void renderWidget(GuiGraphics gui, int mx, int my, float pt) {
        ...
        this.addRenderableWidget(new TransparentButton(10, 200, 110, 20, Component.literal(""), b -> {
            LightningRadarSettings.INSTANCE.sortClosest = !LightningRadarSettings.INSTANCE.sortClosest;
            this.cachedFilteredList = null;
        }) {
            @Override public void renderWidget(GuiGraphics gui, int mx, int my, float pt) {
        ...
        this.addRenderableWidget(new TransparentButton(this.width - 120, 10, 110, 20, Component.literal("Teleport"), b -> {
            if (this.minecraft.player != null && ClientRadarState.currentlyTrackedLightning != null) {
                getFilteredList().stream().filter(d -> d.id.equals(ClientRadarState.currentlyTrackedLightning)).findFirst().ifPresent(td -> {
            ...
            @Override public void renderWidget(GuiGraphics gui, int mx, int my, float pt) {
                this.active = minecraft.player != null && minecraft.player.isCreative() && ClientRadarState.currentlyTrackedLightning != null;
                super.renderWidget(gui, mx, my, pt);
            }
        });
        ...
        if (isWaitingForResults) {
            ...
            if (filtered.isEmpty() && LightningRadarSettings.INSTANCE.hasPerformedSearch) {
                ...
                for (int i = firstVisibleIndex; i < lastVisibleIndex; i++) {
                    ...
                    renderDragonProfile(guiGraphics, d, rX, rY, d.id.equals(selectedDragonEntry), d.id.equals(ClientRadarState.currentlyTrackedLightning), hover);
                }
    ...
    private void renderDragonProfile(GuiGraphics gui, TrackedDragon d, int x, int y, boolean sel, boolean track, boolean hovered) {
        ...
        int finalColor = track ? 0xFF55FF55 : (sel ? 0xFFFFFF55 : COLOR_LIGHTNING);
        gui.drawString(this.font, (track ? ">> " : "") + "Lightning Dragon", (int)(x / 1.5f), (int)(y / 1.5f), finalColor);
    ...
    private void renderStatsBox(GuiGraphics gui, int x, int y) {
        long total = LightningRadarSettings.INSTANCE.globalResults.size();
        ...
        gui.drawString(this.font, "Lightning Dragons = " + total, sx, sy + 15, COLOR_LIGHTNING);
        ps.popPose();
    }
    ...
    public boolean mouseClicked(double mx, double my, int b) {
        ...
        int maxScroll = getMaxScroll();
        if (maxScroll > 0 && !LightningRadarSettings.INSTANCE.globalResults.isEmpty() && mx >= sBX - 5 && mx <= sBX + 15) {
    ...
    public void tick() {
        super.tick();
        if (isWaitingForResults) {
            if (!LightningRadarSettings.INSTANCE.globalResults.isEmpty()) {
